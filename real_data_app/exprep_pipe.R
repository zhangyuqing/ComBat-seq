...
command_args <- commandArgs(trailingOnly=TRUE)
iterations <- 500
alpha_fdr <- as.numeric(command_args[1])  #seq(from=0, to=0.15, by=0.01)[-1]
...
for(iter in 1:iterations){
  ...
  }, error=function(e){print(sprintf("ERROR at iteration %s", iter))})
}  
# end_time <- Sys.time()
# print(end_time - start_time)
  
tpr_res <- CleanOutput(tpr_mat_lst, alpha=alpha_unadj)
fpr_res <- CleanOutput(fpr_mat_lst, alpha=alpha_unadj)
tprADJ_res <- CleanOutput(tprADJ_mat_lst, alpha=alpha_fdr)
fdrADJ_res <- CleanOutput(fdrADJ_mat_lst, alpha=alpha_fdr)
...
save(stats_out, file=sprintf("./exprep_results/perf_stats_shrinkOff_alpha%s.RData", gsub(".", "", alpha_fdr, fixed=TRUE)))
