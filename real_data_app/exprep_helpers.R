...
## DE pipelines
DEpipe <- function(dat_lst, DE_method, alpha){
  cts <- dat_lst$counts 
  batch <- dat_lst$batch
  group <- dat_lst$group
  ...
  cat("#### Raw count ####\n")
  de_called1 <- DE_method(cts, batch=batch, group=group, include.batch=FALSE, 
                          alpha=alpha, covar_incl=covar_incl)  
  # one-step
  cat("#### One-step edgeR ####\n")
  de_called2 <- DE_method(cts, batch=batch, group=group, include.batch=TRUE, 
                          alpha=alpha, covar_incl=covar_incl)  
  
  # current ComBat 
  cat("#### Current ComBat & lm ####\n")
  de_called3 <- currComBat_lm_DEpipe(cts=cts, batch=batch, group=group, 
                                     alpha=alpha, covar_incl=covar_incl)
  ...
  de_called4 <- DE_method(cts=adj_counts_combatseq, batch=batch, group=group, include.batch=FALSE,
                          alpha=alpha, covar_incl=covar_incl)
  ...
  de_called5 <- DE_method(cts, batch=batch, group=group, include.batch=FALSE, 
                          alpha=alpha, covar_incl=covar_incl, covar=uvseq$W)  
  
  # SVAseq 
  cat("#### SVA-seq ####\n")
  mod1 <- model.matrix(~as.factor(group) + covar_incl)#; mod0 <- cbind(mod1[,1])
  mod0 <- model.matrix(~covar_incl)
  svseq <- svaseq(cts, mod1, mod0, n.sv=1);cat("\n")
  de_called6 <- DE_method(cts=cts, batch=batch, group=group, include.batch=FALSE, 
                          alpha=alpha, covar_incl=covar_incl, covar=svseq$sv)  
...
# edgeR DE pipeline
edgeR_DEpipe <- function(cts, batch, group, include.batch, alpha, covar_incl, covar=NULL){
...
currComBat_lm_DEpipe <- function(cts, batch, group, alpha, covar_incl){
  ...
  de_called <- list(unadj=rownames(cts)[pval_seq < alpha], fdr=rownames(cts)[padj_seq < alpha], 
