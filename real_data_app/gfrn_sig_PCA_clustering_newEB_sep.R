...
#counts=cts_sub; batch=batch_sub; group=group_sub_num
start_time <- Sys.time()
combatseq_sub <- ComBat_seq(cts_sub, batch=batch_sub, group=group_sub_num, 
                            ctrl=list(full_mod=TRUE, shrink=TRUE, gene.subset.n=100))
end_time <- Sys.time()
print(end_time - start_time)
...
eb_plts_sub <- lapply(sort(unique(batch)), function(b){
  #curr_out <- unique(do.call(c, ctrl_obj$outlier.genes[[b]]))
  ShrinkParamPlt(cts_sub, combatseq_sub, b, eb_viz_genes=NULL)
})
