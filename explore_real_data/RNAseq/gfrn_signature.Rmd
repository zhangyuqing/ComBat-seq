...
+ Batch 2 ([GSE83083](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE83083)): 9 replicates of each for 3 types of KRAS mutants and GFP control (GEO accession GSE83083)
...
colData(sigdata)$batch <- as.factor(colData(sigdata)$batch)
```
...
percent_zeros_in_genes <- n_zeros_in_genes / ncol(cts_mat)
percent_zeros_df <- data.frame(genes=1:nrow(cts_mat), value=percent_zeros_in_genes)
...
mean_cts_mat["Controls", "top50 up"] <- mean(counts_batch[ , cond_batch==0])
...
test_cond <- 0
counts_cond <- cts_mat[, group_num==test_cond]
