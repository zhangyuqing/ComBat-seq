...
print(as.data.frame(table(group)))
...
print(c(sum(gene_row_with_zeros), round(sum(gene_row_with_zeros)/nrow(cts_mat),3)))
cat("\n- Number of genes with ONLY 0s:\n")
gene_all_zeros <- apply(cts_mat, 1, function(x){all(x==0)})
print(c(sum(gene_all_zeros), round(sum(gene_all_zeros)/nrow(cts_mat),3)))
...
percent_zeros_df <- data.frame(genes=rownames(sigdata), value=percent_zeros_in_genes)
