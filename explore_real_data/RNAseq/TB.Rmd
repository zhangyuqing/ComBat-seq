...
disp_nonzero_lst <- list(Africa=disp_nonzero_batch_africa, G6=disp_nonzero_batch_G6,
                         India=disp_nonzero_batch_india, Brazil_1=disp_nonzero_batch_bzl1, 
                         Brazil_2=disp_nonzero_batch_bzl2)
...
```{r, echo=FALSE, message=FALSE}
disp_nonzero_df <- data.frame(Batch=c(rep("Africa", length(disp_nonzero_batch_africa)), 
                                      rep("G6", length(disp_nonzero_batch_G6)),
                                      rep("India", length(disp_nonzero_batch_india)),
                                      rep("Brazil_1", length(disp_nonzero_batch_bzl1)),
                                      rep("Brazil_2", length(disp_nonzero_batch_bzl2))),
...
+ We show in another analysis (*TB_run_nonzero_disps.R*) that the relationship between two batch did not change much after removing 0s.
...
+ There isn't an obvious difference in zero fractions between batches (aside from Brazil datasets which are too small).


### Goodness of fit for NB distribution to non-zero portion of genes

It is difficult to evaluate quantitatively the goodness-of-fit of NB distribution on data. Instead, we generate Q-Q plots for 100 randomly selected genes, comparing data with NB distribution using ML estimated parameters. We generate these plots **within G6, for cases and control samples separately**. Figures are stored under {output_dir}/qqplots/.
