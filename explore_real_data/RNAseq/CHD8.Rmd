...
+ Biological fold change comparing average counts is about 2-2.2. 
+ Gene-wise fold change has a wide range (median 3, mean 7).
...
+ Mean batch effect fold change comparing average counts ranges from 1.6-4. 
+ Gene-wise mean batch effect fold change has a wide range (mean 4.8, median 24).


### Batch mean

**Median** gene-wise mean across all batches: 
...
+ Mean batch effect fold change (non-zero portion) comparing average counts ranges from 1.5-3.9 (mean), 1.33-5.9 (median). 
+ O counts did not strongly affect mean batch effect in terms of average counts.


### Batch mean with 0 counts removed 

**Median** gene-wise mean after removing zeros: 
