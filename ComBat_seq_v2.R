...
ComBat_seq <- function(counts, batch, group, full_mod=TRUE){  #, normalize="none"){
  ########  Preparation  ########  
  library(DESeq2)  # require bioconductor 3.7, edgeR 3.22.1
  dds <- DESeqDataSetFromMatrix(counts=counts, design=~Group, colData=data.frame(Group=as.factor(group)))
  ...
  ## Estimate dispersion within each batch
  genewise_disp_lst <- lapply(1:n_batch, function(i){
    if(n_batches[i]==1){
