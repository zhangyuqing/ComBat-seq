...
subtitle: "DE - adjusted power and added comparison to current ComBat"
author: Yuqing Zhang
date: October 5, 2018
...
results_dir <- "~/Google Drive/ComBat_seq/DE_analysis_powerdown"
```
...
tpr10_file <- "tpr_simnoBatch_bio15_batch1_sizes10_10_N10_B_depth5.csv"
fpr10_file <- "fpr_simnoBatch_bio15_batch1_sizes10_10_N10_B_depth5.csv"
tpr20_file <- "tpr_simnoBatch_bio15_batch1_sizes10_10_N20_B_depth5.csv"
fpr20_file <- "fpr_simnoBatch_bio15_batch1_sizes10_10_N20_B_depth5.csv"
...
for(i in 1:length(batch_fold_seq)){
  tpr_file <- sprintf("tpr_simFC_bio15_batch%s_sizes100_10_N10_B_depth5.csv", gsub('.', '', batch_fold_seq[i], fixed=TRUE))
  fpr_file <- sprintf("fpr_simFC_bio15_batch%s_sizes100_10_N10_B_depth5.csv", gsub('.', '', batch_fold_seq[i], fixed=TRUE))
...
for(i in 1:length(size1_seq)){
  tpr_file <- sprintf("tpr_simDisp_bio15_batch15_sizes%s_10_N10_B_depth5.csv", size1_seq[i])
  fpr_file <- sprintf("fpr_simDisp_bio15_batch15_sizes%s_10_N10_B_depth5.csv", size1_seq[i])
...
for(i in 1:length(N_seq)){
  tpr_file <- sprintf("tpr_simNsample_bio15_batch15_sizes100_10_N%s_B_depth5.csv", N_seq[i])
  fpr_file <- sprintf("fpr_simNsample_bio15_batch15_sizes100_10_N%s_B_depth5.csv", N_seq[i])
...
for(i in 1:length(design_seq)){
  tpr_file <- sprintf("tpr_simDesign_bio15_batch15_sizes100_10_N10_%s_depth5.csv", design_seq[i])
  fpr_file <- sprintf("fpr_simDesign_bio15_batch15_sizes100_10_N10_%s_depth5.csv", design_seq[i])
...
for(i in 1:length(depth_seq)){
  tpr_file <- sprintf("tpr_simDepth_bio15_batch15_sizes100_10_N10_B_depth%s.csv", depth_seq[i])
  fpr_file <- sprintf("fpr_simDepth_bio15_batch15_sizes100_10_N10_B_depth%s.csv", depth_seq[i])
...
* In theory, ComBat-seq should out-perform edgeR (including batch as covariate) when there is variance difference across batches. However that's solved by current ComBat already (see *Dispersion differences* tab).
* Why does ComBat-seq mainly reduce FPR but barely improve TPR in balanced design? - ComBat-seq tends to make distribution between biological condition groups closer to each other (though not always, about 1/4 DE genes seem to behave that way...). Quantitative ways to show this? **There is variance batch effect, even though batch FC is set to 1 (no mean batch effect)**.
* R > 3.5.1 on SCC4?
