...
results_dir <- "./DE_fullpipe_complete"
source("~/Dropbox/Work/ComBat_Seq/ComBat-Seq/simulations/visualize_helpers.R")

N_samples <- 20
mean_level_vec <- 1
disp_level_vec <- 1:4
confounding_level_vec <- c(0.5, 0.4, 0.3, 0.2)
coverage_level <- 5  #c(1, 5, 10)
alpha_fdr_sel <- 0.1
sel_method_list <- list(edgeR=c("Base.edgeR", "Batch.edgeR", "OneStep.edgeR", "ComBat.lm", "RUVseq.edgeR", "SVAseq.edgeR", 
                                "ComBatseq.ebON.edgeR", "ComBatseq.ebOFF.edgeR", "ComBatseq.New.edgeR"),
                        DESeq2=c("Base.DESeq2", "Batch.DESeq2", "OneStep.DESeq2", "ComBat.lm", "RUVseq.DESeq2", "SVAseq.DESeq2",  
                                 "ComBatseq.ebON.DESeq2", "ComBatseq.ebOFF.DESeq2", "ComBatseq.New.DESeq2"))
sim_outliers <- FALSE
...
for(i in seq_along(confounding_level_vec)){
  ...
  for(j in seq_along(disp_level_vec)){
    ...
    exp_name <- paste0("simFullpipe", ifelse(sim_outliers, "_simout", ""), "_N", N_samples, 
                       "_meanFC", mean_level_vec[1]*10, "_dispFC", disp_level_vec[j],
...
fdr_lst_edgeR$Method <- factor(fdr_lst_edgeR$Method, levels=c("Base", "Batch", "OneStep", "ComBat.lm", "RUVseq", "SVAseq", 
                                                              "ComBatseq.ebON", "ComBatseq.ebOFF", "ComBatseq.New"))
fdr_lst_edgeR$Method <- plyr::revalue(fdr_lst_edgeR$Method, c("Base"="No Batch Effect"))
png(file.path(results_dir, sprintf("figures/FDR_edgeR_m%s.png", gsub(".", "", mean_level_vec[1], fixed=TRUE))), 
...
png(file.path(results_dir, sprintf("figures/FDR_DESeq2_m%s.png", gsub(".", "", mean_level_vec[1], fixed=TRUE))),
    width=8, height=6, units="in", res=300)
...
png(file.path(results_dir, sprintf("figures/Sens_edgeR_m%s.png", gsub(".", "", mean_level_vec[1], fixed=TRUE))), 
    width=8, height=6, units="in", res=300)
...
png(file.path(results_dir, sprintf("figures/Sens_DESeq2_m%s.png", gsub(".", "", mean_level_vec[1], fixed=TRUE))), 
    width=8, height=6, units="in", res=300)
...
for(i in seq_along(confounding_level_vec)){
  ...
  for(j in seq_along(disp_level_vec)){
    ## Read in result files
    exp_name <- paste0("simFullpipe", ifelse(sim_outliers, "_simout", ""), "_N", N_samples, 
                       "_meanFC", gsub(".", "", mean_level_vec[1], fixed=TRUE), "_dispFC", disp_level_vec[j],
                       "_cnfnd", gsub(".","", confounding_level_vec[i], fixed=T), "_depth", coverage_level)
...
png(file.path(results_dir, sprintf("figures/scatter_edgeR_m%s.png", gsub(".", "", mean_level_vec[1], fixed=TRUE))), 
    width=8, height=6, units="in", res=300)
...
png(file.path(results_dir, sprintf("figures/scatter_DESeq2_m%s.png", gsub(".", "", mean_level_vec[1], fixed=TRUE))), 
    width=8, height=6, units="in", res=300)
