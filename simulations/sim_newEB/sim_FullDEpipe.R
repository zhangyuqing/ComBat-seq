rm(list=ls())
demo <- FALSE #FALSE  # if testing code, set as TRUE; if running simulations, set as FALSE
if(demo){  # local
  setwd("~/Documents/ComBat_seq/DE_analysis_tmp/")
  script_dir <- "~/Dropbox/Work/ComBat_Seq/ComBat-Seq"
  source(file.path(script_dir, "simulations/sim_newEB/sim_FullDEpipe_helpers.R"))
}else{  # running on server
  setwd("/restricted/projectnb/combat/work/yuqingz/ComBat-seq/DE_fullpipe/")
  script_dir <- "./scripts"
  source(file.path(script_dir, "sim_FullDEpipe_helpers.R"))
}
sapply(c("polyester", "Biostrings", "limma", "edgeR", "DESeq2", "sva", "RUVSeq", "MASS", "ggpubr"), require, character.only=TRUE)
source(file.path(script_dir, "ComBat_seq.R"))
ComBat_seq_old <- ComBat_seq; rm(ComBat_seq)
source(file.path(script_dir, "ComBat_seq_newEB.R")) 
...
batch_fold <- as.numeric(command_args[1]) 
disp_fold_level <- as.numeric(command_args[2])  # dispersion of batch 2 is how many times that of batch 1, 1-5
confounding_level <- as.numeric(command_args[3])  # level of confounding, 0-0.5
N_total_sample <- as.numeric(command_args[4])  # 20 / 60
coverage <- as.numeric(command_args[5])  # 1 / 5 / 10
#batch_fold <- 1.5; disp_fold_level <- 5; confounding_level <- 0.4; N_total_sample <- 20; coverage <- 5; 
factor_exam <- ifelse(demo, "Fullpipedemo", "Fullpipe")  #command_args[1]  
sim_outliers <- FALSE #as.logical(command_args[1])
...
iterations <- 10 #number of simulations to run
alpha_unadj <- 0.05
alpha_fdr_seq <- seq(from=0, to=0.15, by=0.01)[-1]
exp_name <- paste0("sim", factor_exam, ifelse(sim_outliers, "_simout", ""), 
...
for(ii in seq_along(alpha_fdr_seq)){
  ...
  for(iter in 1:iterations){
    ...
    ## simulate outliers in counts
    if(sim_outliers){
      cts <- simOutliers(cts, fc_mat=fold_changes, s_mat=size_mat, batch=batch, group=group, 
                         rptx=readspertx, outgprob=0.1, outalpha=1e-08)
      counts_base_indi <- simOutliers(counts_base_indi, fc_mat=fold_changes_base, s_mat=size_mat_base, batch=batch, group=group,
                                      rptx=readspertx, outgprob=0.1, outalpha=1e-08)
    }
    ...
    # On adjusted count - old ComBat-seq (with EB shrinkage)
    adj_counts_combatseq_ebON <- ComBat_seq_old(counts=cts, batch=batch, group=group, shrink=TRUE, shrink.disp=TRUE)
    ...
    # On adjusted count - old ComBat-seq (shrinkage OFF)
    adj_counts_combatseq_ebOFF <- ComBat_seq_old(counts=cts, batch=batch, group=group, shrink=FALSE, shrink.disp=FALSE)
    ...
    # On adjusted count - ComBat-seq (New)
    ctrl_obj <- ComBat_seq_control_gene(cts, batch=batch, group=group)
    adj_counts_combatseq_New <- ComBat_seq(counts=cts, batch=batch, group=group, ctrl=ctrl_obj)
    de_called5_New <- edgeR_DEpipe(counts_mat=adj_counts_combatseq_New$adjust_counts, 
                                   batch=batch, group=group, include.batch=FALSE, alpha.unadj=alpha_unadj, alpha.fdr=alpha_fdr)  
    de_called5_deseq_New <- DESeq2_DEpipe(counts_mat=adj_counts_combatseq_New$adjust_counts, 
                                          batch=batch, group=group, include.batch=FALSE, alpha.unadj=alpha_unadj, alpha.fdr=alpha_fdr)
    ...
    DE_objs <- list(Base.edgeR=de_called01, Base.DESeq2=de_called01_deseq,
                    ...
                    ComBatseq.New.edgeR=de_called5_New, ComBatseq.New.DESeq2=de_called5_deseq_New,
