...
if(demo){  # local
  ...
  setwd("/restricted/projectnb/combat/work/yuqingz/ComBat-seq/DE_libcomp_shrinkOff/")
  script_dir <- ".."
...
for(ii in seq_along(alpha_fdr_seq)){
  ...
  for(iter in 1:iterations){
    ...
    adj_counts_combatseq <- ComBat_seq(counts=cts, batch=batch, group=group, shrink=FALSE, shrink.disp=FALSE)
