...
subtitle: "DE - powerdown - turn off EB shrinkage"
author: Yuqing Zhang
date: March 21, 2019
...
We test how well ComBat-seq performs, compared to using other methods to address batch effect, in differential expression analysis. An example of our study design is:
...
results_dir <- "~/Documents/ComBat_seq/DE_analysis_powerdown_shrinkOff_bio13"
```
...
* Fold changes: Biological - 1.3, Batch - 1.5
...
```{r, echo=FALSE, results='hide'}
FC_df <- t(data.frame(Biological=rep("1.3",4), Batch=c("1", "1.5", "2", "3")))
...
for(i in 1:length(batch_fold_seq)){
  tpr_file <- sprintf("tpr_simFC_bio13_batch%s_sizes100_10_N20_B_depth5.csv", gsub('.', '', batch_fold_seq[i], fixed=TRUE))
  fpr_file <- sprintf("fpr_simFC_bio13_batch%s_sizes100_10_N20_B_depth5.csv", gsub('.', '', batch_fold_seq[i], fixed=TRUE))

  tpr_res <- melt(read.csv(file.path(results_dir, tpr_file)))
  fpr_res <- melt(read.csv(file.path(results_dir, fpr_file)))
  
  p_tpr_lst[[i]] <- ggplot(tpr_res, aes(x=variable, y=value)) +
    geom_boxplot() +
    labs(x="", y="TPR", title=sprintf("Bio FC 1.3, Batch FC %s", batch_fold_seq[i])) +
    theme(axis.text.x=element_text(angle=45, hjust=1)) +
    ylim(range(tpr_res$value))
  p_fpr_lst[[i]] <- ggplot(fpr_res, aes(x=variable, y=value)) +
    geom_boxplot() +
    geom_hline(yintercept=0.05, col="red") +
    labs(x="", y="FPR", title=sprintf("Bio FC 1.3, Batch FC %s", batch_fold_seq[i])) +
    theme(axis.text.x=element_text(angle=45, hjust=1))+
    ylim(range(fpr_res$value))
  
  rm(tpr_file, tpr_res, fpr_file, fpr_res)
}

grid.arrange(p_fpr_lst[[1]], p_fpr_lst[[2]], p_fpr_lst[[3]], p_fpr_lst[[4]],
             p_tpr_lst[[1]], p_tpr_lst[[2]], p_tpr_lst[[3]], p_tpr_lst[[4]], 
             nrow=2, widths=rep(2,4), heights=rep(2,2))
...
for(i in 1:length(size1_seq)){
  tpr_file <- sprintf("tpr_simDisp_bio13_batch15_sizes%s_10_N20_B_depth5.csv", size1_seq[i])
  fpr_file <- sprintf("fpr_simDisp_bio13_batch15_sizes%s_10_N20_B_depth5.csv", size1_seq[i])
...
for(i in 1:length(N_seq)){
  tpr_file <- sprintf("tpr_simNsample_bio13_batch15_sizes100_10_N%s_B_depth5.csv", N_seq[i])
  fpr_file <- sprintf("fpr_simNsample_bio13_batch15_sizes100_10_N%s_B_depth5.csv", N_seq[i])
...
grid.arrange(p_fpr_lst[[1]], p_fpr_lst[[2]], p_fpr_lst[[3]], 
             p_tpr_lst[[1]], p_tpr_lst[[2]], p_tpr_lst[[3]], 
             ncol=3, widths=rep(2,3), heights=rep(2,2))
...
for(i in 1:length(design_seq)){
  tpr_file <- sprintf("tpr_simDesign_bio13_batch15_sizes100_10_N20_%s_depth5.csv", design_seq[i])
  fpr_file <- sprintf("fpr_simDesign_bio13_batch15_sizes100_10_N20_%s_depth5.csv", design_seq[i])
...
grid.arrange(p_fpr_lst[[1]], p_fpr_lst[[2]], 
             p_tpr_lst[[1]], p_tpr_lst[[2]],
...
for(i in 1:length(depth_seq)){
  tpr_file <- sprintf("tpr_simDepth_bio13_batch15_sizes100_10_N20_B_depth%s.csv", depth_seq[i])
  fpr_file <- sprintf("fpr_simDepth_bio13_batch15_sizes100_10_N20_B_depth%s.csv", depth_seq[i])
