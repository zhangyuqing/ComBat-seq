...
#### Experiment set up
exp_name <- "test_EBoutdef_N20_simout" #sprintf("outlier_prob%s", gsub(".", "", p, fixed=TRUE))
...
eb_viz_genes <- sort(sample(1:2000, 300, replace=FALSE))
...
setup_df <- expand.grid(index=1,
                        ...
                        outalpha=10^c(-2,-4,-8),
                        simout=TRUE)  # fraction of genes containing outliers
...
for(ii in 1:nrow(setup_df)){
  ...
  if(as.logical(curr_setup$simout)){
    curr_prefix <- paste(curr_prefix, paste0("outgprob", gsub('.', '', curr_setup$outgprob, fixed=T)),
  ...
  #### Use ComBat-seq (new EB)
  ctrl_obj <- ComBat_seq_control_gene(counts, batch=batch, group=group)
  start_time <- Sys.time()
  combatseq_res_New <- ComBat_seq(counts=counts, batch=batch, group=group, ctrl=ctrl_obj)
  end_time <- Sys.time()
  cat("\nRunning time of ComBat-seq (new EB):\n")
  ...
  # de_adj_ebON <- DEpipe(counts_mat=combatseq_res_New$adjust_counts, batch=batch, group=group, ground_truth_vec=de_ground_truth)
  ...
  eb_plts <- lapply(sort(unique(batch)), function(b){ShrinkParamPlt_newEB(counts, combatseq_res_New, b, eb_viz_genes=NULL, outlier_gene_ind)})
  #cts=counts;combatseq_res=combatseq_res_New;batch_name="A";
  ...
  eb_plts_sub <- lapply(sort(unique(batch)), function(b){ShrinkParamPlt_newEB(counts, combatseq_res_New, b, eb_viz_genes, outlier_gene_ind)})
  ...
  plt_adj_ebON <- PCAPlt(combatseq_res_New$adjust_counts, col_data, "ComBat-Seq (new EB)")
  png(paste0(exp_name, "/figures/PCA_", curr_prefix, ".png"), width=9, height=5, units="in", res=300)
  grid.arrange(plt, plt_adj_ebON, ncol=2) #, plt_adj_ebOFF, ncol=3)
  dev.off()
  
  
  #### What's happening to actually counts?
  gene_plt_ori <- AvgExpPlt(counts, de_lst, curr_setup, "Unadjusted")
  gene_plt_adj_ebON <- AvgExpPlt(combatseq_res_New$adjust_counts, de_lst, curr_setup, "ComBat-Seq (new EB)")
  png(paste0(exp_name, "/figures/AvgCts_", curr_prefix, ".png"), width=7, height=7, units="in", res=300)
  grid.arrange(gene_plt_ori, gene_plt_adj_ebON, nrow=2)
  dev.off()
