...
if(demo){  # local
  ...
  setwd("~/yuqingz/ComBat_seq/DE_EBcomplete_outliers_1example/")
  #setwd("/restricted/projectnb/combat/work/yuqingz/ComBat-seq/DE_EBcomplete_outliers_1example/")
  script_dir <- ".."
...
sim_outliers <- as.logical(command_args[1])
...
disp_fold_level <- 2; confounding_level <- 0.5; N_total_sample <- 20; coverage <- 5; 
factor_exam <- ifelse(demo, "EBdemo", "EBtest")  #command_args[1]  
...
exp_name <- paste0("sim", factor_exam, ifelse(sim_outliers, "_simout", ""), 
                   "_N", N_total_sample, "_dispFC", disp_fold_level, 
...
for(ii in seq_along(alpha_fdr_seq)){
  ...
  for(iter in 1:iterations){
    ...
    adj_counts_combatseq_NEW <- ComBat_seq(counts=cts, batch=batch, group=group, shrink=TRUE, shrink.disp=TRUE)
