...
sapply(c("Rcpp", "ggplot2", "reshape2", "gridExtra"), require, character.only=TRUE)
source("ComBat-Seq/ComBat_seq.R")
source("ComBat-Seq/helper_seq.R")
Rcpp::sourceCpp("ComBat-Seq/mcint.cpp")
set.seed(123)

n_gene_seq <- seq(2000, 14000, 1000)
run_time_seq <- rep(NA, length(n_gene_seq))
i=4
for(i in seq_along(n_gene_seq)){
  ...
  start_time <- Sys.time()
  invisible(capture.output(adj_counts <- ComBat_seq(counts=counts, batch=batch, group=group)))#, gene.subset.n=1000)))#, Cpp=FALSE)))
  end_time <- Sys.time()
  run_time_seq[i] <- end_time - start_time
}

save(run_time_seq, n_gene_seq, file="run_time_cpp.RData")

run_time_df <- data.frame(N_genes=n_gene_seq[1:10], run_time=run_time_seq[1:10])
png("run_time_plot_cpp.png", width=6, height=6, units="in", res=300)
ggplot(data=run_time_df, aes(x=N_genes, y=run_time, group=1)) +
  geom_line()+
  geom_point() +
  labs(title="Running time (C++ version)", 
